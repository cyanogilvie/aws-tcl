...
namespace eval aws {
	...
	proc sigv4 args { #<<<
		...
		# Credential scope <<<
		set fq_credential_scope	[aws amz-date $date]/[string tolower $credential_scope/$sig_service/aws4_request]
		# Credential scope >>>
		...
		if {$variant eq "s3v4"} {
			if {"x-amz-content-sha256" ni [lmap {k v} $headers {set k}]} {
				...
				lappend out_headers x-amz-content-sha256	[aws hash AWS4-HMAC-SHA256 $body]
			}
		}
	...
	#>>>
	proc req {method endpoint path args} { #<<<
		variable ratelimit
